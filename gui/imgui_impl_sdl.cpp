...
ImGui_ImplSDL2_UpdateMousePosAndButtons()
{
	...
	if (io.WantSetMousePos)
		SDL_WarpMouseInWindow(
			g_Window, (int)io.MousePos.x, (int)io.MousePos.y);
	else
	...
	if (g_Window == focused_window) {
		if (g_MouseCanUseGlobalState) {
			// SDL_GetMouseState() gives mouse position seemingly
			// based on the last window entered/focused(?) The
			// creation of a new windows at runtime and
			// SDL_CaptureMouse both seems to severely mess up with
			// that, so we retrieve that position globally. Won't
			// use this workaround when on Wayland, as there is no
	...
	// SDL_CaptureMouse() let the OS know e.g. that our imgui drag outside
	// the SDL window boundaries shouldn't e.g. trigger the OS window resize
	// cursor. The function is only supported from SDL 2.0.4 (released Jan
	// 2016)
...
#define MAP_ANALOG(NAV_NO, AXIS_NO, V0, V1)                                    \
	...
	MAP_BUTTON(ImGuiNavInput_TweakFast,
		   SDL_CONTROLLER_BUTTON_RIGHTSHOULDER); // R1 / RB
	MAP_ANALOG(ImGuiNavInput_LStickLeft,
		   SDL_CONTROLLER_AXIS_LEFTX,
		   -thumb_dead_zone,
		   -32768);
	MAP_ANALOG(ImGuiNavInput_LStickRight,
		   SDL_CONTROLLER_AXIS_LEFTX,
		   +thumb_dead_zone,
		   +32767);
	MAP_ANALOG(ImGuiNavInput_LStickUp,
		   SDL_CONTROLLER_AXIS_LEFTY,
		   -thumb_dead_zone,
		   -32767);
	MAP_ANALOG(ImGuiNavInput_LStickDown,
		   SDL_CONTROLLER_AXIS_LEFTY,
		   +thumb_dead_zone,
		   +32767);
