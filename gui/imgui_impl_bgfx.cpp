...
void
ImGui_Implbgfx_RenderDrawLists(ImDrawData *draw_data)
{
	...
	bgfx::setViewRect(
		g_View, 0, 0, (uint16_t)fb_width, (uint16_t)fb_height);
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		if ((numVertices != bgfx::getAvailTransientVertexBuffer(
			...
			// not enough space in transient buffer, quit drawing
			// the rest...
			break;
		}

		bgfx::allocTransientVertexBuffer(
			&tvb, numVertices, g_VertexLayout);
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				const uint16_t xx = (uint16_t)bx::max(
					pcmd->ClipRect.x, 0.0f);
				const uint16_t yy = (uint16_t)bx::max(
					pcmd->ClipRect.y, 0.0f);
				bgfx::setScissor(
					xx,
					yy,
					(uint16_t)bx::min(pcmd->ClipRect.z,
							  65535.0f) -
						xx,
					(uint16_t)bx::min(pcmd->ClipRect.w,
							  65535.0f) -
						yy);

				bgfx::setState(state);
				bgfx::TextureHandle texture = { (uint16_t)(
					(intptr_t)pcmd->TextureId & 0xffff) };
				bgfx::setTexture(
					0, g_AttribLocationTex, texture);
...
ImGui_Implbgfx_CreateDeviceObjects()
{
	...
	g_AttribLocationTex = bgfx::createUniform("g_AttribLocationTex",
